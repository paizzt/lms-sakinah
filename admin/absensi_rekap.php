...
<div class="content-body" style="margin-top: -20px;">
    ...
    <div class="welcome-banner" style="background: linear-gradient(to right, #FF8C00, #F39C12); color: white; padding: 25px; border-radius: 15px; margin-bottom: 30px; box-shadow: 0 10px 20px rgba(255, 140, 0, 0.2);">
        <h2 style="margin: 0; font-size: 24px;"><i class="fas fa-user-check"></i> Absensi Harian</h2>
        <p style="margin: 5px 0 0 0; opacity: 0.9;">Lihat kehadiran siswa per hari atau lihat rekap total.</p>
    </div>
    ...
    <?php if($kelas_pilih != "" && $mapel_pilih != "") { ?>
        ...
        <div style="margin-bottom: 20px; text-align: right;">
            <a href="absensi_detail.php?id=<?php echo $mapel_pilih; ?>" class="btn-tambah" style="background: #2980b9; color: white; text-decoration: none; padding: 12px 25px; border-radius: 8px; font-weight: bold; display: inline-flex; align-items: center; gap: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <i class="fas fa-chart-bar"></i> LIHAT REKAP TOTAL PERTEMUAN
            </a>
        </div>
            ...
            <div class="table-responsive">
                <table class="table table-striped" style="width: 100%; border-collapse: collapse;">
                    ...
                    <tbody>
                        <?php 
                        $no = 1;
                        $q_siswa = mysqli_query($koneksi, "SELECT users.nama_lengkap, users.id_user 
                                                        FROM siswa_detail 
                                                        JOIN users ON siswa_detail.user_id = users.id_user 
                                                        WHERE siswa_detail.kelas_id='$kelas_pilih' 
                                                        ORDER BY users.nama_lengkap ASC");
                        
                        if(mysqli_num_rows($q_siswa) == 0){ echo "<tr><td colspan='4' align='center'>Tidak ada siswa.</td></tr>"; }
                        ...
                        while($s = mysqli_fetch_array($q_siswa)){
                            ...
                            if($status == 'H') $badge = "<span style='background:#d1f2eb; color:#0e6251; padding:5px 10px; border-radius:15px; font-weight:bold;'>HADIR</span>";
                            elseif($status == 'S') $badge = "<span style='background:#d6eaf8; color:#154360; padding:5px 10px; border-radius:15px; font-weight:bold;'>SAKIT</span>";
                            elseif($status == 'I') $badge = "<span style='background:#fcf3cf; color:#7d6608; padding:5px 10px; border-radius:15px; font-weight:bold;'>IZIN</span>";
                            elseif($status == 'A') $badge = "<span style='background:#fadbd8; color:#78281f; padding:5px 10px; border-radius:15px; font-weight:bold;'>ALPA</span>";
                            else $badge = "<span style='color:#ccc; font-style:italic;'>Belum Absen</span>";
                        ?>
    ...
    <?php } else { ?>
        <div style="text-align: center; padding: 50px; background: white; border-radius: 10px;">
            ...
            <h3 style="color: #555;">Silakan Pilih Kelas & Mapel</h3>
        </div>
    <?php } ?>

...
if($act == 'update_bio'){
    ...
    // Update tabel Users
    mysqli_query($koneksi, "UPDATE users SET nama_lengkap='$nama', email='$email' WHERE id_user='$id_user'");
    
    // Update tabel Siswa_Detail
    mysqli_query($koneksi, "UPDATE siswa_detail SET alamat='$alamat', tempat_lahir='$tempat', tanggal_lahir='$tgl' WHERE user_id='$id_user'");
...
elseif($act == 'update_foto'){
    ...
    if($filename != ""){
        ...
        if(in_array(strtolower($ext), $valid)){
            ...
            // Hapus foto lama (opsional, jika bukan default.jpg)
            // ... kode hapus ...
